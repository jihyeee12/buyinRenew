...
import "slick-carousel/slick/slick-theme.css";
    ...
    const RoomCard = () => {
        ...
        <Slider {...settings}>
            {roomData.map((v) => (
                <div className={styles.roomCardArea} onClick={linkRoom}>
                    <div className={styles.roomCard}>
                        <img className={styles.roomImg} src={v.lodgement_img_url} alt='roomImg' />
                        ...
                        <div className={styles.roomInfo}>
                            ...
                            <div className={styles.hotelPrice}>
                                <div className={styles.interestZone}>
                                    <p className={styles.interest}>관심 <span className={styles.interestCount}>{v.wish_num}</span></p>
                                    <p className={styles.interest}>리뷰 <span className={styles.interestCount}>{v.review_num}</span></p>
                                </div>
                                <div className={styles.priceZone}>
                                    {v.rent_list_price === null ? <><br/><br/></>: <><p className={styles.discounts}><span className={styles.discount}>{v.rent_dc_rate}%</span><span className={styles.discountPrice}>{v.rent_list_price}원</span></p>
                                    <p className={styles.roomPrice}>대실 {v.rent_sale_price}원</p></>}
                                    
                                    <p className={styles.discounts}><span className={styles.discount}>{v.lodge_dc_rate}%</span><span className={styles.discountPrice}>{v.lodge_list_price}원</span></p>
                                    <p className={styles.roomPrice}>숙박 {v.lodge_sale_price}원</p>
                                </div>
                            </div>
                        </div>
                    </div>
